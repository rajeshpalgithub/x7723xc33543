...
<?php $id=$client_id; ?>
<form id="permission_list" method="post" class="form-horizontal" action="<?php echo base_url("admin/submit_rest_class_permission/$id");?>">
       ...
       <?php
		...
		if(!empty($client_permission_data))
		{
			foreach($client_permission_data as $item)
			{
				...
				$module_id=$item['id'];
				$checked='';
				$rs=$this->db->select('*')->where('client_id',$id)->where('module_id',$module_id)->get('rest_class_permission');
				if($rs->num_rows()>0)
				{
