...
function generatePIN($digits = 4){
 ...
 public function InsertCustomerData($post_data)
 {
	 ...
	 $result='';
	 /***********************/
	 $account_role_back_trigger = false; 
	 // if it's end customer is registering it self and email for authentication code not able to send then role back will happen
	 /************************/
	 ...
	 if($token_id=="" || $this->Basic_model->role==4)
	 {
		 $account_role_back_trigger = true;
		 ...
		 $insert_arr=array
		  ...
		  //die();
		   ...
		   if($success)
		   {
			   ...
			   if($success)
			   {
				   $result['account_id']=$sub_unique_id; // return account id account id
				   ...
				   $send_email_array=array(
					...
					// Send SMS
					$sms_send_error=false;
				    $send_sms_result=$this->Common_model->send_sms($mobile_no,$final_message);
					if($send_sms_result['error'])
					{
						$sms_send_error=true;
					...
					$send_email_result=$this->Common_model->send_email($send_email_array);
					if($send_email_result['error'])
					{
						$email_send_error=true;
						// $result['successMessage']="Data successfully inserted, Auth code has been send to your register mobile & email id.";
			 			
					}
					if($account_role_back_trigger)
					{
						if($sms_send_error && $email_send_error)
						{
							$error = true;
							$errortext .= $send_sms_result['errortext'].'<br>'.$send_email_result['errortext'];
							// delete account
 ...
 public function GetCustomerData($keyword)
 {
	 $customer_list=array();
	 $client_id= $this->Basic_model->unique_id;
	 $rs=$this->db->select('*')->where('client_id',$client_id)->like('name',$keyword)->get('account');
	 if($rs->num_rows()>0)
	 {
 ...
 public function GetProductCommon()
 {
	 ...
	 {
		 $search_text=$this->input->get('product_type');
		 $search_condition.=" And type='$search_text'"; 
	 }
