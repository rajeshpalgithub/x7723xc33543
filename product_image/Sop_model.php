...
function generatePIN($digits = 4){
		   ...
		   if($success)
		   {
			   ...
			   if($success)
			   {
				   ...
				   $send_email_array=array(
					...
					{
						$result['successMessage']="Data successfully inserted, Auth code has been send to your email id";
	  ...
	  if($rs->num_rows()>0)
	   {
		  $product_data=$rs->row_array();
			  ...
			  $product_array=array(
				...
				'image'=>$product_image['result']['image_list'],
			  );
 ...
 public function InsertBillingAddress($data_array)
 {
	...
	if(!empty($data_array))
	{
		...
		{
			$rs=$this->db->select('*')->where('client_id',$this->Basic_model->unique_id)->where('id',$item['account_id'])->get('account');
			if(!$rs->num_rows()>0)
			{
				$error=true;
				$errortext='Invalid account';
 ...
 public function GetOffRentList($offset,$perpage)
 {
	 ...
	 $sql="SELECT * FROM off_rent WHERE client_id=$client_id $search_condition";
	 
	 if($search_category=='customer_id')
	 {
	   $sql="SELECT * FROM off_rent INNER JOIN account ON off_rent.customer_id=account.id WHERE 
	   off_rent.client_id=$client_id AND account.name LIKE '%$search_text%'";
	 }
	 ...
	 if($rs->num_rows()>0)
	 {
		  ...
		  foreach($offrent_details as $item)
		  {
			  $details=array(
				...
				'order_id'=>$item['order_id'],
				'customer_name'=>$this->Common_model->get_single_field_value('account','name','id',$item['customer_id']),
				'status'=>$this->Common_model->get_single_field_value('off_rent_status_master','order_status_text','id',$item['status_id'])
			   );
 ...
 public function GetServiceRequestList($offset,$perpage)
 {
	 ...
	 $sql="SELECT * FROM service_request WHERE client_id=$client_id $search_condition";
...
return array('error' => $error, 'errortext' => $errortext, 'result' => $result); 
 ...
 public function SubmitOffrent($post_data)
 {
	...
	{
		$sql="Select * from off_rent_status_master INNER JOIN  off_rent_status ON off_rent_status_master.id=off_rent_status.master_status_id  
		Where off_rent_status_master.client_id=$client_id AND off_rent_status_master.id=$status_id";
		$rs=$this->db->query($sql);
		if(!$rs->num_rows()>0)
		{
			$error=true;
	...
	if(!$error)
	{
		...
		if($response==1)
		{
			...
			$rs=$this->db->select('*')->where('id',$order_id)->update('product_order',$order_arr);
 ...
 public function SubmitServiceRequest($post_data)
 {
	...
	{
		$sql="Select * from service_request_status_master INNER JOIN service_request_status ON service_request_status_master.id=service_request_status.master_status_id  
		Where service_request_status_master.client_id=$client_id AND service_request_status_master.id=$status_id";
		$rs=$this->db->query($sql);
		if(!$rs->num_rows()>0)
		{
			$error=true;
 ...
 {
	 $response=0;
	 $is_changeable=$this->Common_model->get_single_field_value('off_rent_status_master','is_changeable','id',$status_id);
	 //echo $this->db->last_query();
	 $client_id=$this->Basic_model->unique_id;
	 if($is_changeable==0)
	 {
		 ...
		 {
			$status_flag=1; 
			$sql="SELECT * FROM off_rent WHERE client_id=$client_id AND order_id=$order_id";
			$rs=$this->db->query($sql);
			$offrent_array=$rs->result_array();
			foreach($offrent_array as $item)
			{
			  $is_changeable=$this->Common_model->get_single_field_value('off_rent_status_master','is_changeable','id',$item['status_id']);
			  if($is_changeable==1)
			  {
				  $status_flag=0;
				  break;
  ...
  public function DeleteFromServiceRequest($post_data)
  {
   ...
   public function UpdateServiceRequest($post_data)
   {
	...
	$service_request_item=$post_data['service_request_item'];
	$order_id=$post_data['order_id'];
	$customer_id=$cust_fname=$this->Common_model->get_single_field_value('product_order','customer_id','id',$order_id);;
